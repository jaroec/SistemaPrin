...
from app.db.models import Sale, CashMovement, MovementType
...
def executive_dashboard(
    ...
    ingresos = db.query(func.coalesce(func.sum(CashMovement.amount_usd), 0)).filter(
        CashMovement.type == MovementType.INGRESO
    ).scalar()

    egresos = db.query(func.coalesce(func.sum(CashMovement.amount_usd), 0)).filter(
    ...
    return {
        ...
        "saldo_neto": float(ingresos - egresos)
    }
